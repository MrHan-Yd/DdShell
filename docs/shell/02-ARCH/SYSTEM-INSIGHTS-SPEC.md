...
## 12. Session Health Score (FR-37)
### 12.1 Purpose
- Provide a single, actionable quality signal per SSH session.
- Help users decide whether to keep operating, retry, or reconnect.

### 12.2 Inputs
- `latencyMsP95` (rolling 60s)
- `packetLossPct` (rolling 60s)
- `reconnectCount5m`
- `commandTimeoutCount5m`

### 12.3 Score Formula
- Normalize each metric to 0~100 penalty and apply weights.
- `score = 100 - (0.40*latencyPenalty + 0.30*lossPenalty + 0.20*reconnectPenalty + 0.10*timeoutPenalty)`
...
### 12.5 Contract Extension
Add to metrics snapshot:
- `sessionHealth.score`: number (0~100)
- `sessionHealth.level`: `GOOD | FAIR | POOR`
- `sessionHealth.reasons`: string[] (top 1~2 causes)
