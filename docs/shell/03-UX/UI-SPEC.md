...
### Quick Edit（SFTP 内轻量编辑）
- 入口：远程文件右键菜单 `快速编辑`；后续可支持双击文本文件直接打开。
- 形态：SFTP 页内大号 modal / sheet，不新增一级导航。
- 布局：
  - 顶部栏：文件名、完整路径、只读/未保存状态、关闭按钮
  - 工具栏：保存、重新加载、查找
  - 主编辑区：等宽字体、行号、轻量文本编辑
  - 底部状态栏：编码、大小、修改时间、保存状态
- 状态要求：支持 `loading / ready / saving / readonly / conflict / error`。
- 交互约束：
  - `Cmd/Ctrl + S` 保存
  - `Cmd/Ctrl + F` 打开查找
  - 关闭前如有未保存内容，必须确认
  - 保存失败不得清空当前草稿
- 风格要求：
  - 继承 macOS feel：glass surface、hairline border、轻阴影、克制动效
  - 不得退化成普通后台表单弹窗
...
- Quick Edit 中的建议动作如需回填到终端，必须遵循“仅回填不自动执行”。
