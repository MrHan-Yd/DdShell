...
- 不做插件系统与插件市场。
- 不做通用 IDE 式内置远端编辑器（不做项目级多文件代码工作区、LSP、Git 集成等）。
- 支持轻量远程文本快速编辑（Quick Edit），用于配置文件、小型脚本和文本文件的查看/编辑/保存。
...
- Quick Edit：在 SFTP 文件管理页直接打开远程文本文件，进行轻量编辑、保存、只读预览与冲突提示。
...
- FR-45 Quick Edit：支持在 SFTP 文件管理页对远程文本文件进行轻量快速编辑；首版提供打开、编辑、保存、只读回退、冲突检测与错误提示，后续补充查找替换、最近编辑、提权保存与建议动作。
