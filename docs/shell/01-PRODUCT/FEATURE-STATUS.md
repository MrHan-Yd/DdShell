...
- 本文件于 `2026-05-01` 更新；除特别说明外，状态基于 `app/` 与 `app/src-tauri/` 静态代码审阅结果维护。
...
- 系统监控：IN_PROGRESS
- 命令中心：IN_PROGRESS
- 命令宏（FR-44 / Workflow Recipe）：DONE
- Quick Edit（FR-45）：IN_PROGRESS
- 自部署同步：TODO
...
- 命令宏：Recipe CRUD、参数定义、顺序步骤执行、运行时参数输入校验、最近运行历史持久化与查看；已按开发完成更新为 DONE
- Quick Edit：已接入 `SftpPage` 右键入口、单选工具栏入口、双击文本文件打开、最近编辑入口、`QuickEditModal`、`CodeMirror 6` 编辑器、Level 2 文件类型/语法映射、查找替换、跳转到行、状态栏语言/行列/换行/缩进展示、前后端文本读写、CRLF 保留、冲突检测、更安全的临时文件 + rename 保存链路、`sudo` 提权保存 + 备份、保存后建议动作、建议命令复制/回填终端、高风险配置文件提示与提权保存 symlink 保护；仍缺完整手工回归记录，保持 `IN_PROGRESS`
