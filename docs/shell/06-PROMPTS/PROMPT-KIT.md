...
## 6. FR-37~FR-43 Task Prompt Snippets
- FR-37 Session Health Score:
  - Define metric inputs, penalty mapping, weighted formula, and UI states (`GOOD/FAIR/POOR`).
  - Output must include data contract, event updates, and non-blocking UX behavior.
- FR-38 Recording Search/Export:
  - Define index fields (sessionId/time range/keyword), clip export format, and privacy masking.
- FR-39 SSH Config Import:
  - Parse `~/.ssh/config` (`Host`, `HostName`, `User`, `Port`, `ProxyJump`, `IdentityFile`) with validation and conflict handling.
- FR-40 Folder Sync:
  - Define incremental scan, ignore rules, conflict policy, dry-run preview, and rollback behavior.
- FR-41 Structured Output Extraction:
  - Extract `IP/path/error-code/URL` from terminal output and map to quick actions.
- FR-42 Monitoring Alerts:
  - Define threshold config model, debounce/cooldown strategy, and alert channels (toast/status bar).
- FR-43 Unified Task Center:
  - Unify transfer/update/retry/reconnect tasks with lifecycle (`queued/running/failed/done/canceled`).
