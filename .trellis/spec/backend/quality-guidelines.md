...
- Tauri config: `bundle.windows.wix.upgradeCode` (pinned; no `fragmentPaths` — see MSI contract below).
...
- Install-type detection: `get_install_type` command → `current_package_type()` → `tauri::utils::platform::bundle_type()` (build-time stamp) with exe-path fallback.
- MSI manual-install chain: `check_update` → `download_update` (emits `update:download_progress/completed/failed`) → `open_installer`.
...
- Windows in-app update flow is split by installer family (detected via `get_install_type` / `bundle_type()`):
  - NSIS: official updater `downloadAndInstall()` with `installMode: "quiet"` — silent, no elevation needed, auto-relaunch after user-confirmed restart. This is the only fully-automatic Windows path.
  - MSI: NEVER silent-install. `msiexec /quiet` runs unelevated via `ShellExecuteW` and per-machine MSI requires elevation, so quiet mode fails silently after the app has already exited. Instead: download the `.msi` via `download_update` to the user's Downloads dir, stop at the `downloadedManualInstall` terminal state, and let the user click to `open_installer` (full interactive wizard + UAC; user picks the install dir in the wizard).
- MSI packages must NOT embed script custom actions. The former `wix-preserve-install-dir.wxs` (inline VBScript, ~170 lines) caused Windows Installer error 2503 ("Setup Wizard ended prematurely") on Windows 11 — script CAs are blocked/broken by script-engine policy and are deprecated by Microsoft. Custom install-path preservation is now delegated to the interactive wizard (user re-picks the dir on manual update). If path preservation is ever re-added, use declarative WiX (`RegistrySearch`/`Property`), never `Script="vbscript"`.
- Windows NSIS updater install mode must stay non-interactive (`quiet`); only the NSIS family reaches the official updater install step. User confirmation belongs in the app UI before install/relaunch.
...
- Frontend install-type cache: cache only successful `get_install_type` results; a failed IPC probe must return `"unknown"` uncached, otherwise an MSI user can be permanently misrouted onto the silent path.
...
- MSI bundle contains any script custom action (`Script="vbscript"`/`"jscript"`) -> interactive install risks error 2503; must fail Windows MSI smoke (inspect CustomAction table).
- MSI user triggers in-app update -> flow must stop at `downloadedManualInstall` with the installer path; silent install of an `.msi` is a defect.
...
- Good: NSIS update preserves `shell.db*` in the install directory and installs silently with auto-relaunch; MSI update downloads to the user's Downloads dir, shows `downloadedManualInstall`, and the user completes the interactive wizard (picking their own install dir).
- Bad: `windows-x86_64` fallback points to NSIS while MSI is also shipped.
- Bad: MSI update path calls the official updater's silent install, or the MSI embeds a VBScript custom action to preserve install paths (2503 regression).
...
- `cargo test --manifest-path app/src-tauri/Cargo.toml msi_install` verifies MSI install-directory installer args remain quoted and stable (Windows-cfg-gated; used by the NSIS-family `configure_windows_updater_builder` path).
- Windows MSI smoke: inspect the built MSI's CustomAction table (Orca or `msiexec /l*v`) — no script custom actions; interactive install completes without 2503.
- Windows MSI in-app update smoke: old MSI build → in-app update → download completes → `downloadedManualInstall` prompt → wizard installs new version.
...
- Windows NSIS manual smoke must install an old version to a non-C path, create `shell.db` data, run in-app update, and verify the install path and data survive. (MSI path preservation is user-driven via the wizard; only verify data survives if the user reinstalls to the same dir.)
...
Wrong (MSI silent install — fails unelevated after the app exits):

```typescript
// installType === "msi" still falls through to the official updater
await pendingUpdate.downloadAndInstall(onEvent); // msiexec /quiet dies silently
```

Correct (MSI stops at a user-driven terminal state):

```typescript
if ((await currentInstallType()) === "msi") {
  await downloadMsiForManualInstall(set, get); // → status: "downloadedManualInstall"
  return; // user clicks → openInstaller(path) → interactive wizard + UAC
}
```
