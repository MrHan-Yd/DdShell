...
fn builtin_commands() -> Vec<BuiltinCommand> {
    vec![
        ...
        BuiltinCommand { title_zh: "校验文件完整性", title_en: "Verify checksums", command: "md5sum -c ", desc_zh: "根据校验文件验证文件完整性", desc_en: "Verify files against checksum file", distro: "common" },

        // ── Common: 文件信息与状态 ──
        BuiltinCommand { title_zh: "文件详细状态", title_en: "Stat file", command: "stat ", desc_zh: "查看文件详细状态（大小、权限、时间戳）", desc_en: "Show detailed file status", distro: "common" },
        BuiltinCommand { title_zh: "查看权限数字", title_en: "Stat octal perms", command: "stat -c '%a %n' ", desc_zh: "以数字形式显示文件权限", desc_en: "Show file permissions in octal", distro: "common" },
        BuiltinCommand { title_zh: "块设备列表", title_en: "List block devices", command: "lsblk -f", desc_zh: "列出块设备及文件系统信息", desc_en: "List block devices with filesystems", distro: "common" },
        BuiltinCommand { title_zh: "解析符号链接", title_en: "Resolve symlink", command: "readlink -f ", desc_zh: "解析符号链接的真实路径", desc_en: "Resolve real path of symlink", distro: "common" },
        BuiltinCommand { title_zh: "提取文件名", title_en: "Basename", command: "basename ", desc_zh: "从路径中提取文件名", desc_en: "Extract filename from path", distro: "common" },
        BuiltinCommand { title_zh: "提取目录路径", title_en: "Dirname", command: "dirname ", desc_zh: "从路径中提取目录部分", desc_en: "Extract directory from path", distro: "common" },
        BuiltinCommand { title_zh: "磁盘/文件复制", title_en: "DD copy", command: "dd if= of= bs=1M status=progress", desc_zh: "块级复制文件或磁盘（显示进度）", desc_en: "Block-level copy with progress", distro: "common" },
        BuiltinCommand { title_zh: "清空文件内容", title_en: "Truncate file", command: "truncate -s 0 ", desc_zh: "将文件截断为0字节（不删除文件）", desc_en: "Truncate file to zero bytes", distro: "common" },
        BuiltinCommand { title_zh: "创建硬链接", title_en: "Create hard link", command: "ln  link_name", desc_zh: "创建硬链接", desc_en: "Create a hard link", distro: "common" },
        BuiltinCommand { title_zh: "创建临时目录", title_en: "Make temp dir", command: "mktemp -d", desc_zh: "创建临时目录并输出路径", desc_en: "Create a temporary directory", distro: "common" },
        BuiltinCommand { title_zh: "CPU核心数", title_en: "CPU core count", command: "nproc", desc_zh: "显示可用CPU核心数", desc_en: "Show number of CPU cores", distro: "common" },
        BuiltinCommand { title_zh: "查看依赖库", title_en: "List shared libs", command: "ldd ", desc_zh: "查看二进制文件依赖的共享库", desc_en: "List shared library dependencies", distro: "common" },

        // ── Common: 性能诊断 ──
        BuiltinCommand { title_zh: "虚拟内存统计", title_en: "Vmstat", command: "vmstat 1 5", desc_zh: "每秒采样虚拟内存/CPU统计（5次）", desc_en: "Virtual memory stats (1s x5)", distro: "common" },
        BuiltinCommand { title_zh: "磁盘IO统计", title_en: "Iostat", command: "iostat -x 1 5", desc_zh: "每秒采样磁盘IO扩展统计（5次）", desc_en: "Extended disk I/O stats (1s x5)", distro: "common" },
        BuiltinCommand { title_zh: "各核CPU统计", title_en: "Mpstat per-CPU", command: "mpstat -P ALL 1 5", desc_zh: "每秒采样各CPU核心使用率（5次）", desc_en: "Per-CPU usage stats (1s x5)", distro: "common" },
        BuiltinCommand { title_zh: "进程资源统计", title_en: "Pidstat", command: "pidstat 1 5", desc_zh: "每秒采样各进程资源使用（5次）", desc_en: "Per-process resource stats (1s x5)", distro: "common" },
        BuiltinCommand { title_zh: "内存使用统计", title_en: "Sar memory", command: "sar -r 1 5", desc_zh: "每秒采样内存使用统计（5次）", desc_en: "Memory usage statistics (1s x5)", distro: "common" },
        BuiltinCommand { title_zh: "CPU热点分析", title_en: "Perf top", command: "sudo perf top", desc_zh: "实时查看CPU热点函数", desc_en: "Live CPU hotspot profiling", distro: "common" },
        BuiltinCommand { title_zh: "进程内存映射", title_en: "Pmap memory", command: "pmap -x ", desc_zh: "查看进程的内存映射详情", desc_en: "Show process memory map", distro: "common" },
        BuiltinCommand { title_zh: "内存占用排行", title_en: "Top by memory", command: "ps aux --sort=-%mem | head -10", desc_zh: "显示内存占用最高的10个进程", desc_en: "Top 10 processes by memory", distro: "common" },
        BuiltinCommand { title_zh: "CPU占用排行", title_en: "Top by CPU", command: "ps aux --sort=-%cpu | head -10", desc_zh: "显示CPU占用最高的10个进程", desc_en: "Top 10 processes by CPU", distro: "common" },
        BuiltinCommand { title_zh: "交互式磁盘分析", title_en: "Ncdu", command: "ncdu ", desc_zh: "交互式分析目录磁盘占用", desc_en: "Interactive disk usage analyzer", distro: "common" },
        BuiltinCommand { title_zh: "网络流量监控", title_en: "Iftop", command: "sudo iftop -i eth0", desc_zh: "按连接实时监控网络流量", desc_en: "Monitor network traffic by connection", distro: "common" },
        BuiltinCommand { title_zh: "进程网络流量", title_en: "Nethogs", command: "sudo nethogs", desc_zh: "按进程监控网络流量", desc_en: "Monitor network traffic by process", distro: "common" },
        BuiltinCommand { title_zh: "流量统计", title_en: "Vnstat daily", command: "vnstat -d", desc_zh: "查看按天统计的网络流量", desc_en: "Show daily network traffic stats", distro: "common" },
        BuiltinCommand { title_zh: "HTTP压力测试", title_en: "Apache bench", command: "ab -n 1000 -c 10 ", desc_zh: "发起1000次请求（并发10）压测", desc_en: "HTTP benchmark (1000 reqs, 10 concurrent)", distro: "common" },

        // ── Common: 文本处理进阶 ──
        BuiltinCommand { title_zh: "打印指定行范围", title_en: "Sed print range", command: "sed -n '10,20p' ", desc_zh: "打印文件第10到20行", desc_en: "Print lines 10-20 of file", distro: "common" },
        BuiltinCommand { title_zh: "删除匹配行", title_en: "Sed delete lines", command: "sed -i '/pattern/d' ", desc_zh: "删除文件中匹配的行（原地修改）", desc_en: "Delete matching lines in-place", distro: "common" },
        BuiltinCommand { title_zh: "AWK指定分隔符", title_en: "AWK field separator", command: "awk -F: '{print $1}' ", desc_zh: "以冒号分隔提取第一列", desc_en: "Print first column with : separator", distro: "common" },
        BuiltinCommand { title_zh: "AWK列求和", title_en: "AWK sum column", command: "awk '{sum+=$1} END {print sum}' ", desc_zh: "对第一列数值求和", desc_en: "Sum values of first column", distro: "common" },
        BuiltinCommand { title_zh: "扩展正则搜索", title_en: "Grep extended regex", command: "grep -E '' ", desc_zh: "使用扩展正则表达式搜索", desc_en: "Search with extended regex", distro: "common" },
        BuiltinCommand { title_zh: "仅列出匹配文件", title_en: "Grep list files", command: "grep -rl '' .", desc_zh: "递归搜索并只列出匹配的文件名", desc_en: "List matching file names only", distro: "common" },
        BuiltinCommand { title_zh: "仅输出匹配部分", title_en: "Grep only matching", command: "grep -o '' ", desc_zh: "只输出匹配到的文本部分", desc_en: "Output only the matched parts", distro: "common" },
        BuiltinCommand { title_zh: "数值排序", title_en: "Sort numeric", command: "sort -n ", desc_zh: "按数值大小排序", desc_en: "Sort lines numerically", distro: "common" },
        BuiltinCommand { title_zh: "排序去重", title_en: "Sort unique lines", command: "sort -u ", desc_zh: "排序并去除重复行", desc_en: "Sort and remove duplicate lines", distro: "common" },
        BuiltinCommand { title_zh: "按列排序", title_en: "Sort by column", command: "sort -t',' -k2 ", desc_zh: "按CSV第二列排序", desc_en: "Sort by second CSV column", distro: "common" },
        BuiltinCommand { title_zh: "仅显示重复行", title_en: "Uniq duplicates", command: "uniq -d ", desc_zh: "只显示重复出现的行", desc_en: "Show only duplicate lines", distro: "common" },
        BuiltinCommand { title_zh: "按键合并文件", title_en: "Join files", command: "join file1 file2", desc_zh: "按共同字段合并两个已排序文件", desc_en: "Join sorted files on common field", distro: "common" },
        BuiltinCommand { title_zh: "带行号输出", title_en: "Number lines", command: "nl ", desc_zh: "输出文件内容并添加行号", desc_en: "Output file with line numbers", distro: "common" },
        BuiltinCommand { title_zh: "反转每行字符", title_en: "Reverse chars", command: "rev ", desc_zh: "反转每行的字符顺序", desc_en: "Reverse characters of each line", distro: "common" },
        BuiltinCommand { title_zh: "十六进制查看", title_en: "Hex dump", command: "xxd ", desc_zh: "以十六进制查看文件内容", desc_en: "View file in hexadecimal", distro: "common" },
        BuiltinCommand { title_zh: "Base64编码", title_en: "Base64 encode", command: "base64 ", desc_zh: "对文件内容进行Base64编码", desc_en: "Base64 encode file content", distro: "common" },
        BuiltinCommand { title_zh: "Base64解码", title_en: "Base64 decode", command: "base64 -d ", desc_zh: "对Base64内容解码", desc_en: "Decode Base64 content", distro: "common" },
        BuiltinCommand { title_zh: "编码转换", title_en: "Iconv convert", command: "iconv -f GBK -t UTF-8 ", desc_zh: "将文件从GBK转换为UTF-8编码", desc_en: "Convert file from GBK to UTF-8", distro: "common" },
        BuiltinCommand { title_zh: "换行符转换", title_en: "Dos2unix", command: "dos2unix ", desc_zh: "将Windows换行符转换为Unix格式", desc_en: "Convert CRLF to LF line endings", distro: "common" },

        // ── Common: 网络进阶 ──
        BuiltinCommand { title_zh: "添加IP地址", title_en: "IP addr add", command: "sudo ip addr add /24 dev ", desc_zh: "为网卡添加IP地址", desc_en: "Add IP address to interface", distro: "common" },
        BuiltinCommand { title_zh: "启用网卡", title_en: "Link up", command: "sudo ip link set  up", desc_zh: "启用网络接口", desc_en: "Bring network interface up", distro: "common" },
        BuiltinCommand { title_zh: "禁用网卡", title_en: "Link down", command: "sudo ip link set  down", desc_zh: "禁用网络接口", desc_en: "Bring network interface down", distro: "common" },
        BuiltinCommand { title_zh: "删除路由", title_en: "Route delete", command: "sudo ip route del ", desc_zh: "删除指定路由", desc_en: "Delete a route", distro: "common" },
        BuiltinCommand { title_zh: "本机IP地址", title_en: "Local IP", command: "hostname -I", desc_zh: "显示本机所有IP地址", desc_en: "Show all local IP addresses", distro: "common" },
        BuiltinCommand { title_zh: "域名注册信息", title_en: "Whois lookup", command: "whois ", desc_zh: "查询域名注册信息", desc_en: "Lookup domain registration info", distro: "common" },
        BuiltinCommand { title_zh: "ARP探测", title_en: "Arping", command: "arping -c 4 ", desc_zh: "发送ARP请求探测局域网主机", desc_en: "Probe LAN host via ARP", distro: "common" },
        BuiltinCommand { title_zh: "带宽测试服务端", title_en: "Iperf3 server", command: "iperf3 -s", desc_zh: "启动iperf3带宽测试服务端", desc_en: "Start iperf3 bandwidth test server", distro: "common" },
        BuiltinCommand { title_zh: "带宽测试客户端", title_en: "Iperf3 client", command: "iperf3 -c ", desc_zh: "连接iperf3服务端测试带宽", desc_en: "Test bandwidth against iperf3 server", distro: "common" },
        BuiltinCommand { title_zh: "Socat端口转发", title_en: "Socat forward", command: "socat TCP-LISTEN:8080,fork TCP:localhost:80", desc_zh: "将8080端口流量转发到80端口", desc_en: "Forward port 8080 to port 80", distro: "common" },
        BuiltinCommand { title_zh: "仅显示状态码", title_en: "Curl status code", command: "curl -o /dev/null -s -w '%{http_code}' ", desc_zh: "请求并只输出HTTP状态码", desc_en: "Output only HTTP status code", distro: "common" },
        BuiltinCommand { title_zh: "测响应耗时", title_en: "Curl timing", command: "curl -o /dev/null -s -w 'time_total: %{time_total}\\n' ", desc_zh: "测量HTTP请求总耗时", desc_en: "Measure total request time", distro: "common" },
        BuiltinCommand { title_zh: "带Token请求", title_en: "Curl bearer token", command: "curl -H 'Authorization: Bearer ' ", desc_zh: "发送带Bearer Token的请求", desc_en: "Request with Bearer token", distro: "common" },
        BuiltinCommand { title_zh: "抓包保存文件", title_en: "TCP dump to file", command: "sudo tcpdump -i any -w capture.pcap", desc_zh: "抓取所有接口数据包并保存", desc_en: "Capture packets to pcap file", distro: "common" },
        BuiltinCommand { title_zh: "远程执行命令", title_en: "SSH remote command", command: "ssh user@host ''", desc_zh: "通过SSH在远程主机执行命令", desc_en: "Run command on remote host via SSH", distro: "common" },
        BuiltinCommand { title_zh: "清除主机记录", title_en: "SSH remove known host", command: "ssh-keygen -R ", desc_zh: "从known_hosts中删除主机记录", desc_en: "Remove host from known_hosts", distro: "common" },
        BuiltinCommand { title_zh: "SCP指定端口", title_en: "SCP with port", command: "scp -P 2222  user@host:/path/", desc_zh: "使用自定义端口上传文件", desc_en: "Upload file with custom port", distro: "common" },
        BuiltinCommand { title_zh: "Rsync指定端口", title_en: "Rsync with port", command: "rsync -avz -e 'ssh -p 2222'  user@host:/path/", desc_zh: "通过自定义SSH端口同步文件", desc_en: "Sync files over custom SSH port", distro: "common" },
        BuiltinCommand { title_zh: "Rsync排除同步", title_en: "Rsync exclude", command: "rsync -avz --exclude='node_modules' ", desc_zh: "同步文件并排除指定目录", desc_en: "Sync files excluding patterns", distro: "common" },

        // ── Common: OpenSSL与加密 ──
        BuiltinCommand { title_zh: "查看证书内容", title_en: "OpenSSL view cert", command: "openssl x509 -in  -text -noout", desc_zh: "查看证书的详细内容", desc_en: "View certificate details", distro: "common" },
        BuiltinCommand { title_zh: "测试SSL连接", title_en: "OpenSSL test connection", command: "openssl s_client -connect host:443 -servername host", desc_zh: "测试SSL/TLS连接并查看证书链", desc_en: "Test SSL/TLS connection and cert chain", distro: "common" },
        BuiltinCommand { title_zh: "查看证书有效期", title_en: "OpenSSL cert dates", command: "echo | openssl s_client -connect host:443 2>/dev/null | openssl x509 -noout -dates", desc_zh: "查看远程站点证书有效期", desc_en: "Check remote certificate expiry dates", distro: "common" },
        BuiltinCommand { title_zh: "生成自签名证书", title_en: "OpenSSL self-signed", command: "openssl req -x509 -newkey rsa:4096 -keyout key.pem -out cert.pem -days 365 -nodes", desc_zh: "生成一年期自签名证书", desc_en: "Generate 1-year self-signed certificate", distro: "common" },
        BuiltinCommand { title_zh: "生成随机密钥", title_en: "OpenSSL random hex", command: "openssl rand -hex 32", desc_zh: "生成32字节随机十六进制密钥", desc_en: "Generate 32-byte random hex key", distro: "common" },
        BuiltinCommand { title_zh: "生成密码哈希", title_en: "OpenSSL password hash", command: "openssl passwd -6", desc_zh: "生成SHA-512密码哈希", desc_en: "Generate SHA-512 password hash", distro: "common" },
        BuiltinCommand { title_zh: "GPG加密文件", title_en: "GPG encrypt", command: "gpg -c ", desc_zh: "使用对称加密方式加密文件", desc_en: "Encrypt file with symmetric cipher", distro: "common" },
        BuiltinCommand { title_zh: "GPG解密文件", title_en: "GPG decrypt", command: "gpg -d ", desc_zh: "解密GPG加密的文件", desc_en: "Decrypt GPG encrypted file", distro: "common" },

        // ── Common: 用户与权限进阶 ──
        BuiltinCommand { title_zh: "密码过期策略", title_en: "Chage list", command: "chage -l ", desc_zh: "查看用户密码过期策略", desc_en: "Show password expiry policy", distro: "common" },
        BuiltinCommand { title_zh: "编辑sudo配置", title_en: "Visudo", command: "sudo visudo", desc_zh: "安全地编辑sudoers配置", desc_en: "Safely edit sudoers file", distro: "common" },
        BuiltinCommand { title_zh: "失败登录记录", title_en: "Failed logins", command: "sudo lastb | head -20", desc_zh: "显示最近的失败登录尝试", desc_en: "Show recent failed login attempts", distro: "common" },
        BuiltinCommand { title_zh: "查看ACL权限", title_en: "Get ACL", command: "getfacl ", desc_zh: "查看文件的ACL访问控制列表", desc_en: "Show file access control list", distro: "common" },
        BuiltinCommand { title_zh: "设置ACL权限", title_en: "Set ACL", command: "setfacl -m u:user:rwx ", desc_zh: "为指定用户设置文件ACL权限", desc_en: "Grant user ACL permission on file", distro: "common" },
        BuiltinCommand { title_zh: "设置文件不可变", title_en: "Chattr immutable", command: "sudo chattr +i ", desc_zh: "设置文件不可修改删除", desc_en: "Make file immutable", distro: "common" },
        BuiltinCommand { title_zh: "取消不可变属性", title_en: "Chattr mutable", command: "sudo chattr -i ", desc_zh: "取消文件的不可变属性", desc_en: "Remove immutable attribute", distro: "common" },
        BuiltinCommand { title_zh: "查看文件属性", title_en: "Lsattr", command: "lsattr ", desc_zh: "查看文件的扩展属性", desc_en: "Show file extended attributes", distro: "common" },
        BuiltinCommand { title_zh: "默认权限掩码", title_en: "Umask", command: "umask", desc_zh: "查看默认文件权限掩码", desc_en: "Show default permission mask", distro: "common" },
        BuiltinCommand { title_zh: "查询用户信息", title_en: "Getent passwd", command: "getent passwd ", desc_zh: "查询用户账户信息", desc_en: "Query user account info", distro: "common" },
        BuiltinCommand { title_zh: "查询主机解析", title_en: "Getent hosts", command: "getent hosts ", desc_zh: "查询主机名解析结果", desc_en: "Query hostname resolution", distro: "common" },

        // ── Common: 磁盘与LVM ──
        BuiltinCommand { title_zh: "分区表信息", title_en: "Parted list", command: "sudo parted -l", desc_zh: "显示磁盘分区表信息", desc_en: "Show disk partition tables", distro: "common" },
        BuiltinCommand { title_zh: "创建物理卷", title_en: "PV create", command: "sudo pvcreate ", desc_zh: "创建LVM物理卷", desc_en: "Create LVM physical volume", distro: "common" },
        BuiltinCommand { title_zh: "创建卷组", title_en: "VG create", command: "sudo vgcreate vg0 ", desc_zh: "创建LVM卷组", desc_en: "Create LVM volume group", distro: "common" },
        BuiltinCommand { title_zh: "创建逻辑卷", title_en: "LV create", command: "sudo lvcreate -L 10G -n lv0 vg0", desc_zh: "创建10G逻辑卷", desc_en: "Create 10G logical volume", distro: "common" },
        BuiltinCommand { title_zh: "扩容逻辑卷", title_en: "LV extend", command: "sudo lvextend -r -L +10G ", desc_zh: "扩容逻辑卷并同步调整文件系统", desc_en: "Extend LV and resize filesystem", distro: "common" },
        BuiltinCommand { title_zh: "逻辑卷列表", title_en: "LV list", command: "sudo lvs", desc_zh: "列出所有逻辑卷", desc_en: "List logical volumes", distro: "common" },
        BuiltinCommand { title_zh: "卷组列表", title_en: "VG list", command: "sudo vgs", desc_zh: "列出所有卷组", desc_en: "List volume groups", distro: "common" },
        BuiltinCommand { title_zh: "物理卷列表", title_en: "PV list", command: "sudo pvs", desc_zh: "列出所有物理卷", desc_en: "List physical volumes", distro: "common" },
        BuiltinCommand { title_zh: "调整ext4大小", title_en: "Resize ext4", command: "sudo resize2fs ", desc_zh: "调整ext4文件系统大小", desc_en: "Resize ext4 filesystem", distro: "common" },
        BuiltinCommand { title_zh: "扩容XFS", title_en: "Grow XFS", command: "sudo xfs_growfs ", desc_zh: "扩容XFS文件系统", desc_en: "Grow XFS filesystem", distro: "common" },
        BuiltinCommand { title_zh: "软RAID状态", title_en: "RAID status", command: "cat /proc/mdstat", desc_zh: "查看软件RAID状态", desc_en: "Show software RAID status", distro: "common" },
        BuiltinCommand { title_zh: "RAID详细信息", title_en: "Mdadm detail", command: "sudo mdadm --detail /dev/md0", desc_zh: "查看RAID阵列详细信息", desc_en: "Show RAID array details", distro: "common" },
        BuiltinCommand { title_zh: "创建Swap文件", title_en: "Create swap file", command: "sudo fallocate -l 2G /swapfile && sudo chmod 600 /swapfile && sudo mkswap /swapfile && sudo swapon /swapfile", desc_zh: "创建并启用2G交换文件", desc_en: "Create and enable 2G swap file", distro: "common" },
        BuiltinCommand { title_zh: "磁盘健康检查", title_en: "SMART health", command: "sudo smartctl -a /dev/sda", desc_zh: "查看磁盘SMART健康信息", desc_en: "Show disk SMART health info", distro: "common" },
        BuiltinCommand { title_zh: "磁盘速度测试", title_en: "Disk speed test", command: "sudo hdparm -Tt /dev/sda", desc_zh: "测试磁盘读取速度", desc_en: "Test disk read speed", distro: "common" },
        BuiltinCommand { title_zh: "重新挂载读写", title_en: "Remount read-write", command: "sudo mount -o remount,rw /", desc_zh: "以读写模式重新挂载根分区", desc_en: "Remount root as read-write", distro: "common" },
        BuiltinCommand { title_zh: "绑定挂载", title_en: "Bind mount", command: "sudo mount --bind  ", desc_zh: "将目录绑定挂载到另一位置", desc_en: "Bind mount directory to another path", distro: "common" },

        // ── Common: 系统与硬件 ──
        BuiltinCommand { title_zh: "内核模块列表", title_en: "List modules", command: "lsmod", desc_zh: "列出已加载的内核模块", desc_en: "List loaded kernel modules", distro: "common" },
        BuiltinCommand { title_zh: "加载内核模块", title_en: "Load module", command: "sudo modprobe ", desc_zh: "加载内核模块", desc_en: "Load a kernel module", distro: "common" },
        BuiltinCommand { title_zh: "卸载内核模块", title_en: "Unload module", command: "sudo modprobe -r ", desc_zh: "卸载内核模块", desc_en: "Unload a kernel module", distro: "common" },
        BuiltinCommand { title_zh: "内存硬件信息", title_en: "Memory hardware", command: "sudo dmidecode -t memory", desc_zh: "查看内存条硬件信息", desc_en: "Show memory hardware details", distro: "common" },
        BuiltinCommand { title_zh: "主机硬件信息", title_en: "System hardware", command: "sudo dmidecode -t system", desc_zh: "查看主机硬件信息", desc_en: "Show system hardware details", distro: "common" },
        BuiltinCommand { title_zh: "硬件概览", title_en: "Hardware summary", command: "sudo lshw -short", desc_zh: "显示硬件配置概览", desc_en: "Show hardware configuration summary", distro: "common" },
        BuiltinCommand { title_zh: "开启IP转发", title_en: "Enable IP forward", command: "sudo sysctl -w net.ipv4.ip_forward=1", desc_zh: "临时开启IP转发", desc_en: "Enable IP forwarding temporarily", distro: "common" },
        BuiltinCommand { title_zh: "应用内核参数", title_en: "Sysctl apply", command: "sudo sysctl -p", desc_zh: "应用sysctl.conf中的内核参数", desc_en: "Apply kernel params from sysctl.conf", distro: "common" },
        BuiltinCommand { title_zh: "强制日志轮转", title_en: "Force logrotate", command: "sudo logrotate -f /etc/logrotate.conf", desc_zh: "强制执行日志轮转", desc_en: "Force log rotation", distro: "common" },
        BuiltinCommand { title_zh: "启动错误日志", title_en: "Journal errors", command: "journalctl -p err -b", desc_zh: "显示本次启动的错误级别日志", desc_en: "Show error logs from current boot", distro: "common" },
        BuiltinCommand { title_zh: "NTP同步源状态", title_en: "Chrony sources", command: "chronyc sources", desc_zh: "查看NTP时间同步源状态", desc_en: "Show NTP sync sources status", distro: "common" },
        BuiltinCommand { title_zh: "崩溃转储列表", title_en: "Coredump list", command: "coredumpctl list", desc_zh: "列出进程崩溃转储记录", desc_en: "List process core dumps", distro: "common" },

        // ── Common: Iptables进阶 ──
        BuiltinCommand { title_zh: "清空防火墙规则", title_en: "Iptables flush", command: "sudo iptables -F", desc_zh: "清空所有防火墙规则（谨慎操作）", desc_en: "Flush all firewall rules (use with care)", distro: "common" },
        BuiltinCommand { title_zh: "端口重定向", title_en: "Iptables redirect", command: "sudo iptables -t nat -A PREROUTING -p tcp --dport 80 -j REDIRECT --to-port 8080", desc_zh: "将80端口流量重定向到8080", desc_en: "Redirect port 80 to 8080", distro: "common" },

        // ── Common: 实用工具 ──
        BuiltinCommand { title_zh: "限时执行命令", title_en: "Timeout run", command: "timeout 10 ", desc_zh: "限定命令最多运行10秒", desc_en: "Run command with 10s time limit", distro: "common" },
        BuiltinCommand { title_zh: "命令计时", title_en: "Time command", command: "time ", desc_zh: "统计命令执行耗时", desc_en: "Measure command execution time", distro: "common" },
        BuiltinCommand { title_zh: "生成数字序列", title_en: "Sequence", command: "seq 1 10", desc_zh: "生成1到10的数字序列", desc_en: "Generate number sequence 1-10", distro: "common" },
        BuiltinCommand { title_zh: "命令行计算", title_en: "Calculator", command: "echo '2+3' | bc", desc_zh: "使用bc进行数值计算", desc_en: "Calculate expression with bc", distro: "common" },
        BuiltinCommand { title_zh: "当前时间戳", title_en: "Unix timestamp", command: "date +%s", desc_zh: "显示当前Unix时间戳", desc_en: "Show current Unix timestamp", distro: "common" },
        BuiltinCommand { title_zh: "时间戳转日期", title_en: "Timestamp to date", command: "date -d @ '+%Y-%m-%d %H:%M:%S'", desc_zh: "将Unix时间戳转换为日期", desc_en: "Convert Unix timestamp to date", distro: "common" },
        BuiltinCommand { title_zh: "显示日历", title_en: "Calendar", command: "cal", desc_zh: "显示当月日历", desc_en: "Show current month calendar", distro: "common" },
        BuiltinCommand { title_zh: "切换默认版本", title_en: "Update alternatives", command: "sudo update-alternatives --config ", desc_zh: "切换命令的默认版本", desc_en: "Switch default command version", distro: "common" },
        BuiltinCommand { title_zh: "录制终端会话", title_en: "Record session", command: "script session.log", desc_zh: "录制终端会话到文件", desc_en: "Record terminal session to file", distro: "common" },
        BuiltinCommand { title_zh: "重置终端", title_en: "Reset terminal", command: "reset", desc_zh: "重置混乱的终端显示", desc_en: "Reset garbled terminal display", distro: "common" },
        BuiltinCommand { title_zh: "重载进程配置", title_en: "Kill HUP", command: "kill -HUP ", desc_zh: "发送SIGHUP信号使进程重载配置", desc_en: "Send SIGHUP to reload process config", distro: "common" },
        BuiltinCommand { title_zh: "调整进程优先级", title_en: "Renice", command: "renice -n 5 -p ", desc_zh: "调整运行中进程的优先级", desc_en: "Change priority of running process", distro: "common" },
        BuiltinCommand { title_zh: "Gzip压缩文件", title_en: "Gzip compress", command: "gzip -9 ", desc_zh: "以最高压缩比压缩文件", desc_en: "Compress file with max compression", distro: "common" },
        BuiltinCommand { title_zh: "Gzip解压文件", title_en: "Gunzip", command: "gunzip ", desc_zh: "解压gzip压缩的文件", desc_en: "Decompress gzip file", distro: "common" },
        BuiltinCommand { title_zh: "查看压缩文件", title_en: "Zcat view", command: "zcat ", desc_zh: "不解压直接查看gzip文件内容", desc_en: "View gzip content without extracting", distro: "common" },
        BuiltinCommand { title_zh: "解压到指定目录", title_en: "Tar extract to dir", command: "tar -xzf  -C ", desc_zh: "解压归档到指定目录", desc_en: "Extract archive to target directory", distro: "common" },
        BuiltinCommand { title_zh: "比较目录差异", title_en: "Diff directories", command: "diff -r dir1 dir2", desc_zh: "递归比较两个目录的差异", desc_en: "Recursively compare two directories", distro: "common" },
        ...
        BuiltinCommand { title_zh: "Docker暂停容器", title_en: "Docker pause", command: "docker pause ", desc_zh: "暂停容器内所有进程", desc_en: "Pause all processes in container", distro: "common" },
        BuiltinCommand { title_zh: "Docker恢复容器", title_en: "Docker unpause", command: "docker unpause ", desc_zh: "恢复已暂停的容器", desc_en: "Unpause a paused container", distro: "common" },
        BuiltinCommand { title_zh: "Docker重命名容器", title_en: "Docker rename", command: "docker rename  ", desc_zh: "重命名容器", desc_en: "Rename a container", distro: "common" },
        BuiltinCommand { title_zh: "Docker创建容器", title_en: "Docker create", command: "docker create --name  ", desc_zh: "创建容器但不启动", desc_en: "Create container without starting", distro: "common" },
        BuiltinCommand { title_zh: "Docker附加到容器", title_en: "Docker attach", command: "docker attach ", desc_zh: "附加到运行中容器的主进程", desc_en: "Attach to running container's main process", distro: "common" },
        BuiltinCommand { title_zh: "Docker等待容器退出", title_en: "Docker wait", command: "docker wait ", desc_zh: "阻塞等待容器退出并输出退出码", desc_en: "Wait for container exit and print exit code", distro: "common" },
        BuiltinCommand { title_zh: "Docker限制容器资源", title_en: "Docker update resources", command: "docker update --memory 512m --cpus 1 ", desc_zh: "更新运行中容器的资源限制", desc_en: "Update resource limits of running container", distro: "common" },
        BuiltinCommand { title_zh: "Docker设置自动重启", title_en: "Docker update restart", command: "docker update --restart unless-stopped ", desc_zh: "设置容器自动重启策略", desc_en: "Set container restart policy", distro: "common" },
        BuiltinCommand { title_zh: "Docker环境变量运行", title_en: "Docker run with env", command: "docker run -d -e KEY=value --name  ", desc_zh: "运行容器并设置环境变量", desc_en: "Run container with environment variable", distro: "common" },
        BuiltinCommand { title_zh: "Docker指定网络运行", title_en: "Docker run with network", command: "docker run -d --network  --name  ", desc_zh: "运行容器并加入指定网络", desc_en: "Run container in specified network", distro: "common" },
        BuiltinCommand { title_zh: "Docker限制资源运行", title_en: "Docker run with limits", command: "docker run -d --memory 512m --cpus 1 --name  ", desc_zh: "运行容器并限制内存和CPU", desc_en: "Run container with memory and CPU limits", distro: "common" },
        BuiltinCommand { title_zh: "Docker自动重启运行", title_en: "Docker run restart always", command: "docker run -d --restart always --name  ", desc_zh: "运行容器并设置总是自动重启", desc_en: "Run container with always restart policy", distro: "common" },
        BuiltinCommand { title_zh: "Docker停止所有容器", title_en: "Docker stop all", command: "docker stop $(docker ps -q)", desc_zh: "停止所有运行中的容器", desc_en: "Stop all running containers", distro: "common" },
        BuiltinCommand { title_zh: "Docker删除所有容器", title_en: "Docker rm all", command: "docker rm -f $(docker ps -aq)", desc_zh: "强制删除所有容器（谨慎操作）", desc_en: "Force remove all containers (use with care)", distro: "common" },
        ...
        BuiltinCommand { title_zh: "Docker容器IP", title_en: "Docker container IP", command: "docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' ", desc_zh: "查看容器的IP地址", desc_en: "Show container IP address", distro: "common" },
        BuiltinCommand { title_zh: "Docker带时间戳日志", title_en: "Docker logs timestamps", command: "docker logs -f -t ", desc_zh: "实时跟踪日志并显示时间戳", desc_en: "Follow logs with timestamps", distro: "common" },
        BuiltinCommand { title_zh: "Docker最近时间日志", title_en: "Docker logs since", command: "docker logs --since 1h ", desc_zh: "查看最近1小时的容器日志", desc_en: "Show container logs from last hour", distro: "common" },
        BuiltinCommand { title_zh: "Docker事件监听", title_en: "Docker events", command: "docker events", desc_zh: "实时监听Docker事件流", desc_en: "Stream real-time docker events", distro: "common" },
        BuiltinCommand { title_zh: "Docker以root进入", title_en: "Docker exec as root", command: "docker exec -it -u root  /bin/bash", desc_zh: "以root用户进入容器终端", desc_en: "Enter container shell as root", distro: "common" },
        ...
        BuiltinCommand { title_zh: "Docker搜索镜像", title_en: "Docker search", command: "docker search ", desc_zh: "在Docker Hub搜索镜像", desc_en: "Search images on Docker Hub", distro: "common" },
        BuiltinCommand { title_zh: "Docker导出容器", title_en: "Docker export", command: "docker export -o container.tar ", desc_zh: "导出容器文件系统为tar", desc_en: "Export container filesystem to tar", distro: "common" },
        BuiltinCommand { title_zh: "Docker导入容器", title_en: "Docker import", command: "docker import container.tar ", desc_zh: "从tar导入为镜像", desc_en: "Import tar as image", distro: "common" },
        BuiltinCommand { title_zh: "Docker镜像详情", title_en: "Docker image inspect", command: "docker image inspect ", desc_zh: "查看镜像详细信息", desc_en: "Inspect image details", distro: "common" },
        BuiltinCommand { title_zh: "Docker悬空镜像", title_en: "Docker dangling images", command: "docker images -f dangling=true", desc_zh: "列出悬空（无标签）镜像", desc_en: "List dangling (untagged) images", distro: "common" },
        BuiltinCommand { title_zh: "Docker指定平台构建", title_en: "Docker build platform", command: "docker build --platform linux/amd64 -t  .", desc_zh: "构建指定架构平台的镜像", desc_en: "Build image for specific platform", distro: "common" },
        BuiltinCommand { title_zh: "Docker构建传参", title_en: "Docker build arg", command: "docker build --build-arg KEY=value -t  .", desc_zh: "构建镜像并传递构建参数", desc_en: "Build image with build arguments", distro: "common" },
        BuiltinCommand { title_zh: "Docker指定Dockerfile构建", title_en: "Docker build file", command: "docker build -f Dockerfile.dev -t  .", desc_zh: "使用指定Dockerfile构建镜像", desc_en: "Build with specific Dockerfile", distro: "common" },
        BuiltinCommand { title_zh: "Docker多架构构建", title_en: "Docker buildx build", command: "docker buildx build --platform linux/amd64,linux/arm64 -t  --push .", desc_zh: "构建多架构镜像并推送", desc_en: "Build multi-arch image and push", distro: "common" },
        BuiltinCommand { title_zh: "Docker构建器列表", title_en: "Docker buildx ls", command: "docker buildx ls", desc_zh: "列出buildx构建器实例", desc_en: "List buildx builder instances", distro: "common" },
        BuiltinCommand { title_zh: "Docker清理构建缓存", title_en: "Docker builder prune", command: "docker builder prune -f", desc_zh: "清理构建缓存", desc_en: "Remove build cache", distro: "common" },
        BuiltinCommand { title_zh: "Docker镜像清单", title_en: "Docker manifest inspect", command: "docker manifest inspect ", desc_zh: "查看镜像清单（多架构信息）", desc_en: "Inspect image manifest (multi-arch)", distro: "common" },
        ...
        BuiltinCommand { title_zh: "Docker登出仓库", title_en: "Docker logout", command: "docker logout", desc_zh: "登出Docker仓库", desc_en: "Logout from docker registry", distro: "common" },
        BuiltinCommand { title_zh: "Docker连接网络", title_en: "Docker network connect", command: "docker network connect  ", desc_zh: "将容器连接到网络", desc_en: "Connect container to network", distro: "common" },
        BuiltinCommand { title_zh: "Docker断开网络", title_en: "Docker network disconnect", command: "docker network disconnect  ", desc_zh: "将容器从网络断开", desc_en: "Disconnect container from network", distro: "common" },
        BuiltinCommand { title_zh: "Docker删除网络", title_en: "Docker network rm", command: "docker network rm ", desc_zh: "删除Docker网络", desc_en: "Remove a docker network", distro: "common" },
        BuiltinCommand { title_zh: "Docker清理网络", title_en: "Docker network prune", command: "docker network prune -f", desc_zh: "清理未使用的网络", desc_en: "Remove unused networks", distro: "common" },
        BuiltinCommand { title_zh: "Docker数据卷详情", title_en: "Docker volume inspect", command: "docker volume inspect ", desc_zh: "查看数据卷详细信息", desc_en: "Inspect volume details", distro: "common" },
        BuiltinCommand { title_zh: "Docker删除数据卷", title_en: "Docker volume rm", command: "docker volume rm ", desc_zh: "删除Docker数据卷", desc_en: "Remove a docker volume", distro: "common" },
        BuiltinCommand { title_zh: "Docker清理数据卷", title_en: "Docker volume prune", command: "docker volume prune -f", desc_zh: "清理未使用的数据卷", desc_en: "Remove unused volumes", distro: "common" },
        ...
        BuiltinCommand { title_zh: "Compose停止服务", title_en: "Docker compose stop", command: "docker compose stop", desc_zh: "停止Compose服务（不删除）", desc_en: "Stop compose services without removing", distro: "common" },
        BuiltinCommand { title_zh: "Compose启动服务", title_en: "Docker compose start", command: "docker compose start", desc_zh: "启动已停止的Compose服务", desc_en: "Start stopped compose services", distro: "common" },
        BuiltinCommand { title_zh: "Compose一次性运行", title_en: "Docker compose run", command: "docker compose run --rm  ", desc_zh: "一次性运行服务命令（完成后删除）", desc_en: "Run one-off command in service", distro: "common" },
        BuiltinCommand { title_zh: "Compose查看进程", title_en: "Docker compose top", command: "docker compose top", desc_zh: "查看Compose服务内的进程", desc_en: "Show processes in compose services", distro: "common" },
        BuiltinCommand { title_zh: "Compose扩容服务", title_en: "Docker compose scale", command: "docker compose up -d --scale =3", desc_zh: "将服务扩容到3个实例", desc_en: "Scale service to 3 instances", distro: "common" },
        BuiltinCommand { title_zh: "Compose指定文件", title_en: "Docker compose file", command: "docker compose -f docker-compose.prod.yml up -d", desc_zh: "使用指定Compose文件启动", desc_en: "Start with specific compose file", distro: "common" },
        BuiltinCommand { title_zh: "Compose最近日志", title_en: "Docker compose logs tail", command: "docker compose logs --tail 100 ", desc_zh: "查看服务最近100行日志", desc_en: "Show last 100 lines of service logs", distro: "common" },
        ...
        BuiltinCommand { title_zh: "Docker上下文列表", title_en: "Docker context ls", command: "docker context ls", desc_zh: "列出Docker上下文（本地/远程）", desc_en: "List docker contexts (local/remote)", distro: "common" },
        BuiltinCommand { title_zh: "Docker切换上下文", title_en: "Docker context use", command: "docker context use ", desc_zh: "切换到指定Docker上下文", desc_en: "Switch to a docker context", distro: "common" },

        // ── Docker: Swarm ──
        BuiltinCommand { title_zh: "Swarm初始化", title_en: "Docker swarm init", command: "docker swarm init", desc_zh: "初始化Swarm集群", desc_en: "Initialize a swarm cluster", distro: "common" },
        BuiltinCommand { title_zh: "Swarm加入集群", title_en: "Docker swarm join", command: "docker swarm join --token  :2377", desc_zh: "以工作节点加入Swarm集群", desc_en: "Join swarm cluster as worker", distro: "common" },
        BuiltinCommand { title_zh: "Swarm获取令牌", title_en: "Docker swarm join-token", command: "docker swarm join-token worker", desc_zh: "获取工作节点加入令牌", desc_en: "Get worker join token", distro: "common" },
        BuiltinCommand { title_zh: "Swarm离开集群", title_en: "Docker swarm leave", command: "docker swarm leave --force", desc_zh: "离开Swarm集群", desc_en: "Leave the swarm cluster", distro: "common" },
        BuiltinCommand { title_zh: "Swarm节点列表", title_en: "Docker node ls", command: "docker node ls", desc_zh: "列出Swarm集群节点", desc_en: "List swarm cluster nodes", distro: "common" },
        BuiltinCommand { title_zh: "Swarm创建服务", title_en: "Docker service create", command: "docker service create --name  --replicas 3 -p 8080:80 ", desc_zh: "创建3副本的Swarm服务", desc_en: "Create swarm service with 3 replicas", distro: "common" },
        BuiltinCommand { title_zh: "Swarm服务列表", title_en: "Docker service ls", command: "docker service ls", desc_zh: "列出Swarm服务", desc_en: "List swarm services", distro: "common" },
        BuiltinCommand { title_zh: "Swarm服务详情", title_en: "Docker service ps", command: "docker service ps ", desc_zh: "查看服务的任务分布", desc_en: "Show service task distribution", distro: "common" },
        BuiltinCommand { title_zh: "Swarm服务日志", title_en: "Docker service logs", command: "docker service logs -f ", desc_zh: "实时跟踪Swarm服务日志", desc_en: "Follow swarm service logs", distro: "common" },
        BuiltinCommand { title_zh: "Swarm服务扩容", title_en: "Docker service scale", command: "docker service scale =5", desc_zh: "将服务扩容到5个副本", desc_en: "Scale service to 5 replicas", distro: "common" },
        BuiltinCommand { title_zh: "Swarm更新镜像", title_en: "Docker service update", command: "docker service update --image  ", desc_zh: "滚动更新服务镜像", desc_en: "Rolling update service image", distro: "common" },
        BuiltinCommand { title_zh: "Swarm删除服务", title_en: "Docker service rm", command: "docker service rm ", desc_zh: "删除Swarm服务", desc_en: "Remove a swarm service", distro: "common" },
        BuiltinCommand { title_zh: "Stack部署", title_en: "Docker stack deploy", command: "docker stack deploy -c docker-compose.yml ", desc_zh: "通过Compose文件部署Stack", desc_en: "Deploy stack from compose file", distro: "common" },
        BuiltinCommand { title_zh: "Stack列表", title_en: "Docker stack ls", command: "docker stack ls", desc_zh: "列出已部署的Stack", desc_en: "List deployed stacks", distro: "common" },
        BuiltinCommand { title_zh: "Stack服务列表", title_en: "Docker stack services", command: "docker stack services ", desc_zh: "列出Stack中的服务", desc_en: "List services in a stack", distro: "common" },
        BuiltinCommand { title_zh: "Stack删除", title_en: "Docker stack rm", command: "docker stack rm ", desc_zh: "删除Stack及其服务", desc_en: "Remove stack and its services", distro: "common" },
        BuiltinCommand { title_zh: "Swarm创建密钥", title_en: "Docker secret create", command: "docker secret create  ", desc_zh: "从文件创建Swarm密钥", desc_en: "Create swarm secret from file", distro: "common" },
        BuiltinCommand { title_zh: "Swarm密钥列表", title_en: "Docker secret ls", command: "docker secret ls", desc_zh: "列出Swarm密钥", desc_en: "List swarm secrets", distro: "common" },
